...
export default (controller: Controller, options?: UseFloatingUIOptions) => {
    const { floatingElement, referenceElement, ...floatingOptions } =
        Object.assign({}, DEFAULT_OPTIONS, options);

    const floating: HTMLElement = (floatingElement ||
        controller.element) as HTMLElement;
    ...
    const detach = () => {
        if (unsubscribe) {
            ...
            safeCallMethod(controller, 'onFloatingAttached', {
    ...
    const attach = () => {
        detach();
        controller.application.logDebugActivity('use-floating-ui', 'attach', {
            reference: referenceElement,
            floating,
        });
        computeElementPosition(false);
        unsubscribe = autoUpdate(referenceElement as Element, floating, () =>
