...
export default class extends Controller {
    ...
    buttonTargetConnected(element: HTMLElement): void {
        ...
        for (const [method, events] of Object.entries(((TRIGGER_TYPES[this.triggerTypeValue] || {})))) {
            events.forEach((event) => {
                element.addEventListener(event, () => safeCallMethod(this, method));
            })
        }
    }
    ...
    connect() {
        ...
        if (this.triggerTypeValue == "hover") {
            for (const [method, events] of Object.entries(TRIGGER_TYPES.hover)) {
                events.forEach((event) => {
                    this.element.addEventListener(event, () => safeCallMethod(this, method));
                })
            }
        }
    }
