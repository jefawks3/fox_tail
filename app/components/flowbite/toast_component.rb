...
class Flowbite::ToastComponent < Flowbite::DismissibleComponent
    ...
    icon: {
      ...
      renders: lambda { |icon, options = {}, &block|
        render_visual block ? capture(&block) : nil, options do |attributes|
          render Flowbite::IconBaseComponent.new(icon, attributes)
        end
      }
    },
    svg: {
      ...
      renders: lambda { |path, options = {}, &block|
        render_visual block ? capture(&block) : nil, options do |attributes|
          render Flowbite::InlineSvgComponent.new(path, attributes)
        end
      }
    },
    image: {
  ...
  renders_one :action, lambda { |options = {}|
    ...
    Flowbite::ClickableComponent.new options
  }
