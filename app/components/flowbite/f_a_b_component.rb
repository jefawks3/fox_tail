...
class Flowbite::FABComponent < Flowbite::BaseComponent
  ...
  renders_many :items, lambda { |icon_or_options = {}, options = {}|
    if icon_or_options.is_a?(Hash)
      Flowbite::FAB::ItemComponent.new merge_item_options(icon_or_options)
    else
      Flowbite::FAB::ItemComponent.new icon_or_options, merge_item_options(options)
    end
  }

  renders_one :icon, lambda { |icon_or_options = {}, options = {}|
    if icon_or_options.is_a? Hash
      Flowbite::IconButtonComponent.new merge_fab_options(icon_or_options)
    else
      Flowbite::IconButtonComponent.new icon_or_options, merge_fab_options(options)
    end
  }
