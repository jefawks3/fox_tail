...
class Flowbite::FAB::ItemComponent < Flowbite::IconButtonComponent
  ...
  def before_render
    ...
    if label_style == :tooltip && self.class.use_stimulus?
      trigger_component = Flowbite::TooltipTriggerComponent.new tag_id, "##{tooltip_id}", trigger_type: :hover
      options = trigger_component.stimulus_controller_options
      attributes = Flowbite::TooltipTriggerComponent.stimulus_controller.attributes options
  ...
  def render_tooltip
    render Flowbite::TooltipComponent.new(tooltip_id, placement: label_placement, trigger_id: tag_id).with_content(content)
  end
end
